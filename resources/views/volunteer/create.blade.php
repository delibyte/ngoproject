<x-layout>
    <x-setting :heading="'Apply For Volunteership'" class="w-10/12 mx-auto">
        <form id="volunteerForm" method="POST" action="{{ route('volunteer.application.store') }}" enctype="multipart/form-data">
            @csrf

            <x-form.input name="name" :value="$user->name" disabled />
            <x-form.input name="profession" :value="old('profession')" required />
            ...
            <x-form.field>
                ...
                <select name="transportation" required class="ml-2 p-2 rounded-md">
                        <option value="yes" {{ old('transportation') == "yes" ? 'selected' : '' }}
                        >Yes</option>
                        <option value="no" {{ old('transportation') == "no" ? 'selected' : '' }}
                        >No</option>
                </select>
